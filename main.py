...
MENUTEXTHEIGHT = 50
MENUOPTION1 = 'N - New game'
MENUOPTION2 = 'Eacape - Quit'
...
class Bubble(pygame.sprite.Sprite):
    ...
    def update(self):
        # Оновлення позиції кульки на кожній ітерації гри
        ...
        if self.angle == 90:
            ...
            x = self.xcalculate(self.angle)
            y = self.ycalculate(self.angle)
        elif self.angle > 90:
            x = self.xcalculate(180 - self.angle) * -1
            y = self.ycalculate(180 - self.angle)
    ...
    def xcalculate(self, angle):
        # Обчислення руху по осі X залежно від кута
    ...
    def ycalculate(self, angle):
        # Обчислення руху по осі Y залежно від кута
...
class Arrow(pygame.sprite.Sprite):
    ...
    def update(self, direction):
        # Оновлення кута стрілки відповідно до напрямку руху
...
class Score(object):
    def __init__(self):
        # Ініціалізація об'єкта для рахунку очків
    ...
    def update(self, deleteList):
        # Оновлення рахунку очків

        self.total += ((len(deleteList)) * 10)
        self.render = self.font.render('Score: ' + str(self.total), True, BLACK, WHITE)

    def draw(self):
        # Малювання рахунку очків на екрані
...
def getBlankBoard():
    ...
    for X in range(WINDOWWIDTH):
        board.append([None] * WINDOWHEIGHT)
    return board
...
def showMenu():
    ...
    font = pygame.font.SysFont('Helvetica', MENUTEXTHEIGHT)
    titleText = font.render('Bubble Shooter!', True, BLACK)
    titleRect = titleText.get_rect()
    titleRect.centerx = DISPLAYSURF.get_rect().centerx
    titleRect.top = 50

    option1Text = font.render(MENUOPTION1, True, BLACK)
    option1Rect = option1Text.get_rect()
    option1Rect.centerx = DISPLAYSURF.get_rect().centerx
    option1Rect.top = 200

    option2Text = font.render(MENUOPTION2, True, BLACK)
    option2Rect = option2Text.get_rect()
    option2Rect.centerx = DISPLAYSURF.get_rect().centerx
    option2Rect.top = 250

    while True:
        DISPLAYSURF.fill(WHITE)
        DISPLAYSURF.blit(titleText, titleRect)
        DISPLAYSURF.blit(option1Text, option1Rect)
        DISPLAYSURF.blit(option2Text, option2Rect)
...
def makeBlankBoard():
    # Створює пусту дошку з кульками
...
def setBubbles(array, gameColorList):
    # Розташовує кульки на дошці з кольорами зі списку gameColorList
...
def setArrayPos(array):
    # Встановлює позиції кульки на дошці
...
def deleteExtraBubbles(array):
    # Видаляє кульки, які виходять за межі вікна
...
def updateColorList(bubbleArray):
    # Оновлює список кольорів на основі кульок на дошці
...
def checkForFloaters(bubbleArray):
    # Перевіряє, чи є кульки, які "висять" у повітрі (не мають кульок-сусідів)
...
def popFloaters(bubbleArray, copyOfBoard, column, row=0):
    # Рекурсивно видаляє "висячі" кульки
...
def stopBubble(bubbleArray, newBubble, launchBubble, score):
    # Зупиняє кульку, перевіряє зіткнення з іншими кульками та видаляє групи кульок
...
def addBubbleToTop(bubbleArray, bubble):
    # Додає кульку на верхню позицію в масиві кульок
...
def popBubbles(bubbleArray, row, column, color, deleteList):
    # Рекурсивно видаляє групи кульок одного кольору
...
def makeDisplay():
    # Створює вікно для відображення гри
...
def terminate():
    # Завершує програму і виходить з неї
...
def coverNextBubble():
    # Показує наступну кульку, якою гравець стрілятиме
...
def endScreen(score, winorlose):
    # Відображає екран завершення гри з результатом
...
def main():
    ...
    choice = showMenu()
    # Запускаємо гру, якщо користувач вибирає "New Game"

    while choice == 'newgame':
        getBlankBoard()
